...
// ----------------------------------------------
// Utility Functions (RNG, TEA, LCG, Fresnel)
// ----------------------------------------------
...
// ----------------------------------------------
fn sample_disk(seed: ptr<function, u32>) -> vec2f {
    // Uniform random points in a disk using polar coordinates
    // r = sqrt(U), theta = 2*pi*V, U and V ~ U(0,1)
    let U = rnd(seed);
    let V = rnd(seed);

    let r = sqrt(U);
    let theta = 2.0 * 3.14159 * V;
    let x = r * cos(theta);
    let y = r * sin(theta);
    return vec2f(x, y);
}
...
fn get_camera_ray(ipcoords: vec2f, seed: ptr<function, u32>) -> Ray {
    ...
    let v = normalize(p - e);        // viewing direction
    let b1 = normalize(cross(v, u)); // right vector
    let b2 = cross(b1, v);           // up vector
    
    let x = ipcoords[0];
    let y = ipcoords[1];
    let q = v*d + (b1 * x) + (b2 * y);

    // Focus distance setup
    // We assume that 'aperture' (uniforms_f.aperture) controls the lens radius.
    // For a real DOF, you'd relate aperture to aperture and focal length.
    // Here we simplify: aperture acts like an aperture scale.
    let focus_plane_dist = length(p - e) * uniforms_f.focus_dist; // focusing at point p
    let lens_radius = uniforms_f.aperture * 15; // scale lens radius by aperture

    // Sample a point on the lens disk
    let lens_uv = sample_disk(seed) * lens_radius;
    let lens_offset = lens_uv.x * b1 + lens_uv.y * b2;

    // The focus point on the focus plane
    // Ray should converge at e+v*focus_plane_dist
    let focus_point = e + v * focus_plane_dist + (b1 * x + b2 * y)* (focus_plane_dist/d);

    // New ray origin is offset on the lens
    let new_origin = e + lens_offset;
    let new_direction = normalize(focus_point - new_origin);

    return Ray(new_origin, new_direction, 1e-9, 1e9);
}
...
// Scene intersection (spheres, AABB, trimesh)
...
// ----------------------------------------------
// Shading Models and Light Sampling
// ----------------------------------------------
...
// Area light sampling from a triangle mesh
...
// Glossy shading (example: phong + refract)
...
fn main_fs(@builtin(position) fragcoord: vec4f, @location(0) coords: vec2f) -> FSOut {
  ...
  // Incorporate jitter and DOF by passing seed to get_camera_ray
