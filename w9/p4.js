...
async function main() {
  ...
  const wgsl = device.createShaderModule({
    code: await (await fetch("./p4.wgsl")).text()
  });

  const bg_texture = await loadTexture(device, "../backgrounds/hangar_interior_4k.RGBE.png");
  ...
  const drawingInfo = await readOBJFile(filename, 10, true);
