...
fn intersect_scene(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> bool {
    ...
    if (intersect_triangle(*r, hit, array<vec3f,3>(v0, v1, v2)))
    {
        ...
        if ((*r).tmax == (*hit).dist) 
        {
            ...
            (*hit).ambientColor = triangleRGB*0.9;
        }
    }
...
fn intersect_sphere(r: Ray, hit: ptr<function, HitInfo>, position: vec3f, radius: f32) -> bool 
{
    ...
    if (objectHit) {
        var t = dot(-d, (e - position)) + sqrt(disc) / dot(d, d);
        if ((*hit).dist > 0) {
            (*hit).dist = min((*hit).dist, t);
        }
        else {
...
fn lambertian(r: ptr<function, Ray>, hit: ptr<function, HitInfo>) -> vec3f
{
    ...
    var temp = HitInfo(false, 0.0, vec3f(0.0), vec3f(0.0), vec3f(0.0), vec3f(0.0), 0);
    ...
    {
        return ((*hit).diffuseColor/3.14) * light.L_i * (max(0, dot((*hit).normal, light.w_i)));
    }
}
...
{
    switch (*hit).shader {
        case 1 {return (lambertian(r,hit) + ((*hit).ambientColor * 0.1));}
        case default {return (*hit).ambientColor;}
    }
}
